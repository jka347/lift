...
- **Rest timer defaults live in code, overrides in data**: auto rest countdown on rep taps / Log — `ex.rest` seconds if set (priority slots get 150 in the default program), else 60 for superset lifts, 90 otherwise. `unit: "sec"` exercise timers are different: they count up from zero, record elapsed time when stopped, and auto-stop/notify at `2 × repHigh`. `ensureFormGuides()` also back-fills missing `rest` fields. No timer while editing a past session.
...
- **Body tracking**: `bodyMetrics` is an additive top-level array created on first check-in for existing data; do not migrate or rewrite workout sessions to enable it. Values explicitly use `weightLb` and circumference `*In` fields, independent of exercise units. Home reminders use local dates: 7 days for weight, one clamped calendar month per core measurement (waist/chest/right relaxed arm/right thigh). Hips/right calf are optional. Partial entries keep missing core measurements due. Reminders are in-app only; saves use the existing Gist/offline/conflict path.
- **Body reminder dismissal**: the compact Home banner appears only when a check-in is due. Dismiss stores today's local date in `lift_bodyReminderDismissedOn` on that device and hides it for the rest of that day; it never changes body data or due dates. Body tracking remains accessible from its permanent Home link.
